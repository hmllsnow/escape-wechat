...
import { parseActionNameString,getTalkerAndRoom } from '../Utils.js';
/**
 * 检查消息是否满足指定的规则
...
async function handlePersonalMessage(message, bot, config) {
  ...
  */
  ...
  console.log(JSON.stringify(message.talker()))
  ...
  for (const rule of reply) {
    if (matchesRule(message, rule)) {
      ...
      await handler(bot,message,rule);
      break; // 找到匹配的规则后停止遍历
    }
  }
  ...
  console.log('onMessage.handlePersonalMessage ----> no match exit function handlePersonalMessage')
 ...
 * @param {Object} rule 
 */
async function handler(bot,message,rule) {
  console.log('onMessage.handler 统一处理函数----->entry ')
  const handlerString = rule.handler;
  const [handlerName, params] = parseActionNameString(handlerString)
  const handlerImp = bot[handlerName];
  const inputParams = {params:params}
  let ruleWithParams = Object.assign({},rule,inputParams);
  const talkerInfo = await getTalkerAndRoom(message)
  ruleWithParams = Object.assign({},ruleWithParams,talkerInfo);
  console.log('onMessage.handler 统一处理函数----->ruleWithParams '+JSON.stringify(ruleWithParams))

  if (typeof handlerImp === 'function') {
    await handlerImp(bot,message,ruleWithParams);
  } else {
    console.error(`onMessage.handler function ${handlerName} not found or not implemented.`);
  }
}
